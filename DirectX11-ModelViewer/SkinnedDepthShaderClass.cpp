...
bool SkinnedDepthShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	result = InitializeShader(device, hwnd, L"SkinnedDepthVertexShader.hlsl", L"DepthPixel.hlsl");
	if (!result)
	{
		return false;
	}
...
void SkinnedDepthShaderClass::Shutdown()
{
	ShutdownShader();
}

bool SkinnedDepthShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, const std::vector<XMMATRIX>& bones)
{
	bool result;

	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, bones);
	if (!result)
	{
		return false;
	}
...
bool SkinnedDepthShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void SkinnedDepthShaderClass::ShutdownShader()
{
	if (mMatrixBuffer)
	{
...
void SkinnedDepthShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool SkinnedDepthShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, const std::vector<XMMATRIX>& bones)
{
	HRESULT result;
...
void SkinnedDepthShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
